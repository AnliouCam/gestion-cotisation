...
@section('content')
    <div class="max-w-3xl mx-auto">
        {{-- En-tête --}}
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Modifier la Cotisation</h1>
            <p class="text-gray-600 text-sm mt-1">Modifier les informations de cette cotisation</p>
        </div>
            ...
            <form action="{{ route('cotisations.update', $cotisation) }}" method="POST">
                    ...
                    <select name="utilisateur_id" id="utilisateur_id" required
                        ...
                        @foreach($utilisateurs as $utilisateur)
                            <option value="{{ $utilisateur->id }}"
                                    {{ (old('utilisateur_id', $cotisation->utilisateur_id) == $utilisateur->id) ? 'selected' : '' }}>
                    ...
                    <select name="evenement_id" id="evenement_id" required
                        ...
                        @foreach($evenements as $evenement)
                            <option value="{{ $evenement->id }}"
                                    {{ (old('evenement_id', $cotisation->evenement_id) == $evenement->id) ? 'selected' : '' }}>
                                {{ $evenement->nom }}
                ...
                <div class="mb-6">
                    ...
                    <input type="date" name="date" id="date" required
                           value="{{ old('date', $cotisation->date->format('Y-m-d')) }}"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="commentaire" id="commentaire" rows="3"
                              ...
                              class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('commentaire') border-red-500 @enderror">{{ old('commentaire', $cotisation->commentaire) }}</textarea>
                    @error('commentaire')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6 bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div class="text-sm text-gray-600 space-y-1">
                        <p><strong>Créée par :</strong> {{ $cotisation->createur->nom }}</p>
                        <p><strong>Créée le :</strong> {{ $cotisation->created_at->format('d/m/Y à H:i') }}</p>
                        @if($cotisation->updated_at != $cotisation->created_at)
                            <p><strong>Dernière modification :</strong> {{ $cotisation->updated_at->format('d/m/Y à H:i') }}</p>
                        @endif
                    </div>
                </div>
